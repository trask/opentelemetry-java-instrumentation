...
Real examples from the CHANGELOG (all from alpha instrumentation builder APIs):

| Deprecated (version) | Removed (version) | What changed |
|---|---|---|
| `setSpanNameExtractor()` (2.23.0) | removed (2.24.0) | → `setSpanNameExtractorCustomizer()` |
| `setMessagingReceiveInstrumentationEnabled()` (2.24.0) | removed (2.25.0) | → `setMessagingReceiveTelemetryEnabled()` |
| `new*()` factory methods (2.24.0) | removed (2.25.0) | → `create*()` naming convention |
| `getDbSystem()` / `getResponseStatus()` (2.24.0) | removed (2.25.0) | → `getDbSystemName()` / `getResponseStatusCode()` |
