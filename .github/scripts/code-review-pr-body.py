...
def validate_change(change: object, field_name: str) -> dict[str, object]:
    ...
    validated["line_hint"] = line_hint
    return validated


def validate_unresolved(item: object, field_name: str) -> dict[str, str]:
...
def render_body(
    ...
    lines = [
        f"Automated code review of `{module_dir}`.",
        "",
        f"Model: `{model}`",
        "",
        "### Summary",
    ...
    if changes:
        lines.extend(
            [
                "| File | Category | Change | Reason |",
                "| --- | --- | --- | --- |",
            ]
        )
        for change in changes:
            lines.append(
                "| {file} | {category} | {change_text} | {reason} |".format(
                    file=render_path(change["path"], change["line_hint"]),
...
def main() -> None:
    ...
    body = render_body(report, args.module_dir, args.model, args.run_url)
