...
sed -Ei "s/(opentelemetryJavaagent = )\"[^\"]*\"/\1\"$version\"/" examples/distro/buildSrc/src/main/kotlin/OtelVersions.kt
sed -Ei "s/(opentelemetryJavaagentAlpha = )\"[^\"]*\"/\1\"$alpha_version\"/" examples/distro/buildSrc/src/main/kotlin/OtelVersions.kt

sed -Ei "s/(io.opentelemetry.instrumentation.muzzle-generation\"\) version )\"[^\"]*\"/\1\"$alpha_version\"/" examples/distro/settings.gradle.kts
sed -Ei "s/(io.opentelemetry.instrumentation.muzzle-check\"\) version )\"[^\"]*\"/\1\"$alpha_version\"/" examples/distro/settings.gradle.kts
