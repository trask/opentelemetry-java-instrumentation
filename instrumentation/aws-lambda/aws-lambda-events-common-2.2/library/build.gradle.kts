...
dependencies {
  ...
  // So that is why we add it as compile only dependency.
  library("com.amazonaws:aws-lambda-java-serialization:1.1.5")

  // We need Jackson for wrappers to reproduce the serialization Lambda performs when invoking a RequestHandler with an event
