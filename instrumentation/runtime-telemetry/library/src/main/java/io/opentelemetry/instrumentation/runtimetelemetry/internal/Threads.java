...
public class Threads {
  ...
  public static List<AutoCloseable> registerObservers(Meter meter) {
    return INSTANCE.registerObservers(meter, useThreads());
  }

  private List<AutoCloseable> registerObservers(Meter meter, boolean useThread) {
    if (useThread) {
      return registerObservers(meter, Threads::getThreads);
    }
    return registerObservers(meter, ManagementFactory.getThreadMXBean());
  }

  // Visible for testing
  List<AutoCloseable> registerObservers(Meter meter, ThreadMXBean threadBean) {
  ...
  // Visible for testing
  List<AutoCloseable> registerObservers(Meter meter, Supplier<Thread[]> threadSupplier) {
    return registerObservers(meter, Threads::java8ThreadCallback, threadSupplier);
  }
