...
public abstract class AbstractHttpServerTest<SERVER> extends AbstractHttpServerUsingTest<SERVER> {
    ...
    for (int i = 0; i < count; i++) {
      assertions.add(
          trace -> {
                ...
                span ->
                    span.hasName(rootSpan.getName())
                        ...
                        .hasAttributesSatisfyingExactly(
                            equalTo(longKey(ServerEndpoint.ID_ATTRIBUTE_NAME), requestId)));
            spanAssertions.add(
                span -> assertIndexedBodyServerSpan(span, requestId).hasParent(rootSpan));
            ...
            spanAssertions.add(
                span ->
                    assertIndexedBodyControllerSpan(span, requestId)
                        .hasParent(trace.getSpan(parentIndex)));
            spanAssertions.add(
                span ->
  ...
  @CanIgnoreReturnValue
  protected SpanDataAssert assertIndexedBodyServerSpan(SpanDataAssert span, int requestId) {
    ServerEndpoint endpoint = INDEXED_CHILD_FROM_REQUEST_BODY;
    String method = "POST";
    return assertServerSpan(span, method, endpoint, endpoint.status)
  ...
  @CanIgnoreReturnValue
  protected SpanDataAssert assertIndexedBodyControllerSpan(SpanDataAssert span, int requestId) {
    span.hasName("controller").hasKind(SpanKind.INTERNAL);
    return span;
  }
